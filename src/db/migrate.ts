...
try {
  // Serializes migration attempts when multiple replicas start together.
  await client`select pg_advisory_lock(hashtext('imagecdn_migrations'))`
  await client`
  ...
  let [lastMigration] = await client<{ created_at: string | null }[]>`
    select created_at
  ...
  if (!lastMigration && migrations.length > 0) {
    const [schemaState] = await client<{ complete: boolean }[]>`
      select
  ...
  await client`select pg_advisory_unlock(hashtext('imagecdn_migrations'))`.catch(() => undefined)
  await client.end()
}
